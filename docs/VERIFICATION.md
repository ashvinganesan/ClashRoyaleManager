...
6. The bot links the Discord account to the Clash Royale player, assigns `Verified`, assigns `Elder` or `Co-Leader` when the player's in-game clan role matches, updates nickname, and posts the new-member info embed.
...
When enabled, `/verify player_tag:#PLAYER_TAG` immediately auto-confirms the member if the tag is currently in the configured clan. The bot still assigns `Verified`, applies the matching `Elder` or `Co-Leader` Discord role when applicable, and posts an "Auto Confirmed Clash Royale Verification" audit message in the leader verification channel. Disable it with `/set_auto_verification enabled:false` to return to clan-chat code confirmation.
